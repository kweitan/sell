<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right" style="font-size: 12px;">
      ...
      <el-breadcrumb-item>广告栏编辑</el-breadcrumb-item>
    </el-breadcrumb>
    <div style="height: 15px;"></div>
    <el-form ref="form" :model="goods" label-position="right" :rules="rules" label-width="110px">
      <el-form-item label="广告栏名称" prop="categoryName">
        <el-col :span="12">
      ...
      <el-form-item label="广告栏URL" >
        <el-col :span="12">
          <el-select v-model="goods.bannerUrl" placeholder="请选择广告栏URL">
            <el-option
...
<script>
  ...
  export default {
    ...
    mounted() {
      let bannerInfo = JSON.parse(this.$route.params.bannerInfo) ;
      //this.goods = JSON.parse(categoryInfo) ;
      this.goods.bannerName = bannerInfo.bannerName;
      this.goods.bannerUrl = bannerInfo.bannerUrl;
      this.goods.bannerIcon = bannerInfo.bannerIcon;
      this.goods.bannerId = bannerInfo.bannerId;
      this.goods.hashNumber = bannerInfo.hashNumber;
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$confirm('确定保存数据?', '温馨提示', {
              ...
              //保存
              that.$api.updateBanner(JSON.stringify(that.goods),
                success=>{
